...
class EditPurchases extends Component {
  ...
  getAreasAndFeilds() {
    ...
    return this.viewsService.getEntityAreas(cookies).then(allAreasAndFields => {
      let allAreas = allAreasAndFields.areas.filter(
        area => area.EntityTypes == EntityTypes.Purchase
      );
  ...
  getEntityData() {
    ...
    this.purchasesService.getPurchase(query.id).then(response => {
  ...
  saveEntityData() {
    ...
    this.purchasesService.editPurchase(entity).then(response => {
      this.props.showSaveSpinner(false);
      if (response.data.success) {
        toast.success("Successfully saved purchase.");
      } else {
            ...
            <div className={BoxStyles.box}>
              <div className={BoxStyles["box-body"]}>
                <span style={{ fontSize: "25px" }}>
                  ...
                  <b>
                    {" "}
                    {this.props.majorentity.entityData.PurchaseNumber || ""}
                  </b>
                </span>
              </div>
            </div>
