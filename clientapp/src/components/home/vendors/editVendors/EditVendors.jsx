...
class EditVendors extends Component {
  ...
  getAreasAndFeilds() {
    ...
    return this.viewsService.getEntityAreas(cookies).then(allAreasAndFields => {
      let allAreas = allAreasAndFields.areas.filter(
        area => area.EntityTypes == EntityTypes.Vendor
      );
  ...
  getEntityData() {
    ...
    this.vendorsService.getVendor(query.id).then(response => {
  ...
  saveEntityData() {
    ...
    this.vendorsService.editVendor(entity).then(response => {
            ...
            <div className={BoxStyles.box}>
              <div className={BoxStyles["box-body"]}>
                <span style={{ fontSize: "25px" }}>
                  <b>{this.props.majorentity.entityData.VendorName || ""}</b>
                </span>
              </div>
            </div>
          </div>
